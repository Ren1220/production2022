...
PSInput main(VSInput input)
{
    ...
    // 基準テクセルからＵ座標を+1テクセルからずらすためのオフセットを計算する
    output.texcoord0.xy = float2(1.0f / texcoord_size.x, 0.0f);

    // 基準テクセルからＵ座標を+3テクセルからずらすためのオフセットを計算する
    output.texcoord1.xy = float2(3.0f / texcoord_size.x, 0.0f);

    // 基準テクセルからＵ座標を+5テクセルからずらすためのオフセットを計算する
    output.texcoord2.xy = float2(5.0f / texcoord_size.x, 0.0f);

    // 基準テクセルからＵ座標を+7テクセルからずらすためのオフセットを計算する
    output.texcoord3.xy = float2(7.0f / texcoord_size.x, 0.0f);

    // 基準テクセルからＵ座標を+9テクセルからずらすためのオフセットを計算する
    output.texcoord4.xy = float2(9.0f / texcoord_size.x, 0.0f);

    // 基準テクセルからＵ座標を+11テクセルからずらすためのオフセットを計算する
    output.texcoord5.xy = float2(11.0f / texcoord_size.x, 0.0f);

    // 基準テクセルからＵ座標を+13テクセルからずらすためのオフセットを計算する
    output.texcoord6.xy = float2(13.0f / texcoord_size.x, 0.0f);

    // 基準テクセルからＵ座標を+15テクセルからずらすためのオフセットを計算する
    output.texcoord7.xy = float2(15.0f / texcoord_size.x, 0.0f);
